...
Item {
    ...
    Rectangle {
        ...
        // ---- 面板的"点击兜底" ----
        //
        // **为什么必须有**（实测）：面板里只有顶部箭头槽与各个 chip 有自己的
        // MouseArea，其余白色区域（顶部条右侧、行与行之间的空隙、面板底部）
        // 没有任何东西接收事件 —— 点击会**穿透到下面的海报卡片**，直接打开
        // 某个动漫的详情页。用户想点空白处收起面板，结果进了别的页面。
        //
        // 这里铺一层"只吃点击、不做事"的 MouseArea：accepted 默认 true
        // 即可拦住穿透，`onClicked` 留空（点空白处**不**收起面板 ——
        // 大面板误触收起反而烦人，收起统一走左上角箭头）。
        //
        // z 放在最底层（body 的子项按声明顺序叠放，这里声明得早、
        // 后面的 topStrip / chips 会覆盖其上），因此**不会**抢走
        // 箭头和 chip 的点击。
        MouseArea {
            anchors.fill: parent
            // 收起态时整块是那个圆形按钮，点击由外层的圆形按钮区域处理；
            // 展开后才需要这层兜底（否则会和收起态的点击打架）
            enabled: root.opened
        }
